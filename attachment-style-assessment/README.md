...
**Relationship-context screen:** right after choosing Quick/Full, a short screen asks whether the person is currently partnered, single with past relationship(s), or has never been in a relationship. This doesn't change scoring — it only changes the instructional line shown on each section-intro screen (e.g. "think of your most recent relationship" vs. "answer with how you'd expect yourself to feel"), so the items (which are worded around "partner," matching the ECR-R construct they're inspired by) stay answerable even without a current or past partner. If "never been in a relationship" is chosen, the report adds an extra caveat next to the cutoff explanation noting the score is a rougher approximation, since the underlying instrument is normed on people with actual romantic-relationship experience. State lives in `state.relContext`; content in `UI.relContextQuestion` / `UI.sectionContextNote` / `UI.neverPartneredCaveat` in `js/app.js`.

**Anxiety Resources (conditional section):** when the Anxiety dimension scores High (above the 4.0 midpoint — true for both Anxious-Preoccupied and Fearful-Avoidant), the report adds a "Managing the Anxiety" card with: a few concrete self-help techniques to try immediately (grounding, cognitive reframing, delay-and-soothe); a front-line tier of therapy modalities aimed at present-day symptoms and relationship patterns (EFT, CBT, ACT); and a second, clearly-separated "if it traces back further" tier of modalities aimed at family-of-origin or trauma-rooted patterns (Family Systems Therapy, Internal Family Systems, EMDR, Art Therapy — the last flagged as best used as an adjunct, not standalone). All entries are descriptive ("what it is / why it fits"), never prescriptive, and the section carries its own disclaimer that modality fit is a licensed clinician's call. Content lives in `ANXIETY_RESOURCES` in `js/scoring.js`; rendering is `renderAnxietyResources()` in `js/app.js`.
